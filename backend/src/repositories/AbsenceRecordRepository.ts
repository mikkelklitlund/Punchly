...
class AbsenceRecordRepository {
    ...
    async getAbsenceRecordsByEmployeeIdAndRange(employeeId: number, periodStart: Date, periodEnd: Date): Promise<AbsenceRecordDTO[]> {
        const absences = await this.prisma.absenceRecord.findMany({
            where: {
                employeeId,
                AND: [
    ...
    private mapAbsenceType(absenceType: PrismaAbsenceType): AbsenceType {
        switch (absenceType) {
            ...
            case 'SICK': return AbsenceType.SICK;
            case 'HOMEDAY': return AbsenceType.HOMEDAY;
            case 'PUBLIC_HOLIDAY': return AbsenceType.PUBLIC_HOLIDAY;
            default: throw new Error('Unknown absence type');
        }
    }
}
